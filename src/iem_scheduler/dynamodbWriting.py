...
class DynamoDBCountdown:
    ...
    def parse_captured_content_v2(self, content):
        entries = content.split("\n")
        parsed_entries = []
    ...
    def delete(self, ticketid, login):
        client = boto3.client("dynamodb")

        response = client.delete_item(
            TableName="Countdown_Notification",
            Key={"ticketId": {"S": ticketid}, "login": {"S": login}},
        )

        return response

    def delete_v2(self, ticketid, entry):
        client = boto3.client("dynamodb")
    ...
    def records_comparision_v2(self, ticketid, dbquery, ticketcontent):
        transformed_result = []
        for item in dbquery:
            transformed_item = {k: v["S"] for k, v in item.items()}
            transformed_result.append(transformed_item)
